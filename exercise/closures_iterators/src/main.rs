...
fn main() {
    ...
    let transformed: Vec<String> = words
        .into_iter()
        .filter(|word: &&str| !word.contains('h'))
        .map(|word: &str| word.to_uppercase())
    ...
    let doubles = vec![(0, 1), (2, 3), (4, 5)];
    let mut transformed_doubles = Vec::new();

    for (x, y) in doubles {
        transformed_doubles.push((x + 1, y * 2));
    }
    transformed_doubles
        .into_iter()
        .for_each(|pair| print!("{:?}", pair));

    let mut data = vec![1, 2, 3, 4];
    data.iter_mut().for_each(|x| *x *= 3);
    println!("{:?}", data);
}
